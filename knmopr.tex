...
\foreach \x/\y/\z in {2/1/3,3/0/5,4/0/0} {
	\draw<2-3>[fill=chameleongreen2!30](\x-1,-2.05) rectangle (\x,-1.05);
	\ifthenelse{\x=2}{
		\node<2-3>[onslide=<\z>{text=red}] at (\x-.5,-1.7) {\Large $\texttt{\y}$};
	}{\node<2-3> at (\x-.5,-1.7) {\Large \texttt{\y}};}
    \draw<4-5>[fill=chameleongreen2!30] (\x,-2.05) rectangle (\x+1,-1.05);
    \ifthenelse{\x=3}{
		\node<4-5>[onslide=<\z>{text=red}] at (\x+.5,-1.7) {\Large $\texttt{\y}$};
	}{\node<4-5> at (\x+.5,-1.7) {\Large \texttt{\y}};}
    \draw<6-8>[fill=chameleongreen2!30] (\x+1,-2.05) rectangle (\x+2,-1.05);
...
\node[visible on=<8>] at (p3) {\textcolor{markgreen}{\quad\Large $\checkmark$\normalsize $~s=2$}};
\end{tikzpicture}
\end{center}
\end{frame}
...
%		(q1) edge [bend left,above,out=40,in=140] node {$c$} (q0)
		(q1) edge [distance=1em,loop above] node {$a$} (q1)
%		(q2) edge [bend left,below left,out=30,in=150] node {$b,c$} (q0)
%		(q3) edge [bend right,above,out=320,in=220] node {$c$} (q0)
		(q3) edge [bend right,above right,out=320,in=220] node {$a$} (q1)
%		(q4) edge [bend left,out=40,in=140]node {$b,c$} (q0)
		(q5) edge [bend right,below,out=320,in=220] node {$a$} (q1)
		(q5) edge [bend left,below right,out=30,in=150] node {$b$} (q4)
%		(q6) edge [bend left,out=20,in=140] node {$b,c$} (q0)
		(q7) edge [bend left,out=320,in=220] node {$a$} (q1)
		(q7) edge [bend left,above left,out=25,in=150] node {$b$} (q2)
		;	
\end{tikzpicture}}
\end{center}
\begin{small}
...
\node at (0.3,-2.4) {\footnotesize $j$};
\node<1-4> at (0.3,-0.5) { $t$};
\node<1-4> at (0.3,-1.7) { $p$};
...
\foreach \x/\y in {1/a,2/b,3/c,4/a,5/b,6/d} {
	\draw<1>[fill=chameleongreen2!30] 	(\x,-2.05) rectangle (\x+1,-1.05);
    \node<1> at (\x+.5,-1.7) {\Large \texttt{\y}};
    \node<1> at (\x+.5,-2.4) {\tiny \texttt{\x}};
    \draw<2>[fill=chameleongreen2!30] 	(\x+3,-2.05) rectangle (\x+4,-1.05);
    \node<2> at (\x+3.5,-1.7) {\Large \texttt{\y}};
    \node<2> at (\x+3.5,-2.4) {\tiny \texttt{\x}};
    \draw<3>[fill=chameleongreen2!30] 	(\x+4,-2.05) rectangle (\x+5,-1.05);
    \node<3> at (\x+4.5,-1.7) {\Large \texttt{\y}};
    \node<3> at (\x+4.5,-2.4) {\tiny \texttt{\x}};
    \draw<4>[fill=chameleongreen2!30] 	(\x+7,-2.05) rectangle (\x+8,-1.05);
...
\mathfrak{p}_1&=&\vert aab\vert&=&3,\\
\mathfrak{p}_2&=&\vert aabaab\vert&=&6,\\
\mathfrak{p}_3&=&\vert aabaaba\vert&=&7
\end{IEEEeqnarray*}
\end{frame}
...
\mathfrak{p}_1=9:							&  \red{aab}&\red{aab}&\red{aab}\\
											&&&& \red{aab}&\red{aab}&\red{aab}\\
\mathfrak{p}_2=6:							&  \blue{aab}&\blue{aab}&&&\blue{aab}&\blue{aab}\\
											&&&\blue{aab}&\blue{aab}\\
\mathrm{ggT}(\mathfrak{p}_1,\mathfrak{p}_2)=3:&  \green{aab}&&\green{aab}&&\green{aab}\\
											&&\green{aab}&&\green{aab}&&\green{aab}
\end{IEEEeqnarray*}
...
\draw[fill=chameleongreen2!35] (0,0) rectangle (11,1);
\draw (11,0) rectangle (12,1);
\draw[fill=chameleongreen2] (0,0) rectangle (2,1);
\draw[fill=chameleongreen2] (9,0) rectangle (11,1);
\draw[fill=red!35] (2,0) rectangle (3,1);
\draw (12,1) -- (13,1);
\draw (12,0) -- (13,0);
\node at (12.5,0.5) {\dots};
\node at (-1,0.5) {\LARGE $x$};
\node at (1,0.5) {\Large $v$};
\node at (10,0.5) {\Large $v$};
\node at (2.5,0.5) {\large $\neq a$};
\node at (11.5,0.5) {\Large $a$};

\draw[|-|] (0,-.75) -- (11,-.75) node [midway,fill=white] {\Large $u$};
%\draw [line width=1pt,decorate,decoration={brace,amplitude=10pt,mirror}](0,0) -- (12,0) node[midway,yshift=-2.5em] {\Large $v=p[1\tdots j]$};
\end{tikzpicture}}
\end{center}
\end{frame}
...
%Wir dürfen davon ausgehen, dass $w$ \emph{echtes} Präfix von $p$ ist, da die Anzahl der Vergleiche von $p$ mit $t$ bei einem Vorkommen mit Shift $s$ gleich der Anzahl derer bei einem Mismatch nach dem Vorkommen des größten Randes von $p$, welches auch echtes Präfix von $p$ ist.
\end{frame}
