...
\only<4>{Was ist das größte $k<j$, sodass:\[p[1\tdots k] \text{ ist Rand von } p[1\tdots j]\] gilt, mit $s'=s+(j-k)$? $s'$ ist der nächste Kandidat.}
\end{frame}

\begin{frame}{Präfixfunktion}
Wir suchen also die Länge $k$ des größten Randes von $p[1\tdots j]$ für alle $j$ mit $1\leq j\leq\vert p\vert$.
\end{frame}

\begin{frame}{Präfixfunktion}
\begin{defi}
Sei $p$ ein Wort über $\Sigma$ mit $\vert p\vert=m$ und sei $j\geq 1$. Die \emph{Präfixfunktion} $\pi$ für das Muster $p$ ist definiert durch \[\pi\colon\{1,2,\dots,m\}\to\{0,1,\dots,m-1\}\] mit
\[\pi(j)=\mathrm{Max}\big(k:p[1\tdots k]\text{ ist Rand von }p[1\tdots j]\big)\]
\end{defi}
\end{frame}
...
Wir suchen die Länge $k$ des \emph{größtmöglichen Randes} für alle Präfixe des Musters.\medskip
